...
import { FaStar, FaCheckCircle, FaTimesCircle, FaTruck, FaUndoAlt, FaClock, FaArrowAltCircleLeft } from "react-icons/fa";
...
const {id}= useParams();
  ...
  if (!product) {
    return <div className="text-center mt-20 text-lg"><Loader/></div>;
  }
    ...
    <div className="bg-gradient-to-br from-white to-blue-50 min-h-screen p-6 md:p-12">
      <div className="max-w-7xl mx-auto bg-white rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="grid md:grid-cols-2 gap-10 p-8 relative">
         <Link className="absolute top-4 left-4 z-20" to={"/mainlayout/products"}>
          <FaArrowAltCircleLeft className="text-3xl text-gray-600 hover:text-gray-800 transition cursor-pointer "/></Link>
          <div> 
            <img
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-1 text-yellow-500 text-sm">
              ...
              <span className="text-gray-600 ml-2">{product.rating.toFixed(1)} / 5.0</span>
            </div>

            {/* Pricing */}
            <div className="text-2xl font-bold text-green-600">
              ${product.price}
              <del className="ml-3 text-sm text-gray-400 ">${originalPrice}</del>
              <span className="ml-3 text-sm text-red-600">-{product.discountPercentage}%</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              {product.stock > 0 ? (
                <>
                  ...
                  <span className="text-green-700 text-sm font-semibold">In Stock: {product.stock} items</span>
                </>
              ) : (
                <>
            ...
            {product.meta && (
              <div className="mt-4">
                <h4 className="text-md font-semibold mb-1 text-gray-700">Use Before</h4>
          ...
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-800">{keyword.reviewerName}</span>
            <span className="text-yellow-600">Rating: {keyword.rating}⭐</span>
          </div>
          <p className="text-sm text-gray-700 mt-1">{keyword.comment}</p>
          <p className="text-xs text-gray-500 mt-1 text-right">{(Date(keyword.date).toLocaleString()).slice(3,24)}</p>
        </div>
                        </div>
        ...
        <div className="border-t p-8 bg-blue-50 grid md:grid-cols-2 gap-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">3–7 business days across India.</p>
            </div>
          </div>
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Return within 10 days, no questions asked.</p>
            </div>
          </div>
        </div>
      </div>
